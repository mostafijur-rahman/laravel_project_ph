        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="box box-success bn">
                    <div class="box-body">
                        <div class="row">
                            <form method="get" action="{{ url('trip-final-report') }}">
                                <div class="col-md-3">
                                    <label>তারিখ</label>
                                    <input type="date" class="form-control" name="date" value="{{old('date')}}">
                                </div>
                                <div class="col-md-3">
                                    <label>ট্রিপ নং</label>
                                    <input type="text" class="form-control" name="trip_no" value="{{old('trip_no')}}">
                                </div>
                                <div class="col-md-3">
                                    <label>গাড়ীর নং:</label>
                                    <select  class="form-control select2" style="width: 100%;" name="car_encrypt">
                                        <option value="">নির্বাচন করুন</option>
                                        @if(isset($cars))
                                        @foreach($cars as $car)
                                        <option value="{{ $car->car_encrypt }}" {{ old('car_encrypt')==$car->car_encrypt ? 'selected':'' }}>{{ $car->car_number }}</option>
                                        @endforeach
                                        @endif
                                    </select>
                                </div>
