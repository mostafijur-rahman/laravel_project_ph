                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        @foreach($clients as $key=>$list)
                         @php $due_data = $list->due_trips; @endphp
                            @if(isset($start_date) && isset($end_date))
                                @php $due_data = $due_data->whereBetween('trip_date', [$start_date, $end_date]); @endphp
                            @endif
                        @php
                            $client_total = (count($due_data))? $due_data->sum('trip_due_fair') : 0;
