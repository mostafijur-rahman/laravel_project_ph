@if(
    $trip->company->due_fair > 0
    || $trip->company->demarage_due > 0 
    || ($down_trip && $down_trip->company->due_fair > 0)
    || ($down_trip && $down_trip->company->demarage_due > 0))
<div class="col-md-12">
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" id="transection_action" action="trips/transection-save-for-up-down-challan">
                @csrf
                <input type="hidden" name="trip_id" value="{{ $trip->id }}">
                <input type="hidden" name="down_trip_id" value="{{ ($down_trip)?$down_trip->id:null }}">
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select class="form-control {{ $errors->has('transection_type') ? 'is-invalid' : '' }}" id="transection_type" name="transection_type" required>
                                ...
                                <option value="receive_challan_due_from_company_for_up_challan">@lang('cmn.receive_challan_due_bill_from_company') (@lang('cmn.up_challan'))</option>
                                @endif

                                @if($trip->company->demarage_due > 0)
                                <option value="receive_demurrage_due_from_company_for_up_challan">@lang('cmn.receive_demurrage_due_bill_from_company') (@lang('cmn.up_challan'))</option>
                                @endif

                                @if($down_trip && $down_trip->company->due_fair > 0)
                                <option value="receive_challan_due_from_company_for_down_challan">@lang('cmn.receive_challan_due_bill_from_company') (@lang('cmn.down_challan'))</option>
                                @endif
                                
                                @if($down_trip && $down_trip->company->demarage_due > 0)
                                <option value="receive_demurrage_due_from_company_for_down_challan">@lang('cmn.receive_demurrage_due_bill_from_company') (@lang('cmn.down_challan'))</option>
                                @endif
