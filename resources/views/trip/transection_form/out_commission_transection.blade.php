@if($trip->company->due_fair > 0 || $trip->company->demarage_due > 0 || $trip->provider->due_fair > 0 || $trip->provider->demarage_due > 0)
<div class="col-md-12">
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" id="transection_action" action="trips/transection-save-for-out-commission-transection">
                @csrf
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select class="form-control {{ $errors->has('transection_type') ? 'is-invalid' : '' }}" id="transection_type" name="transection_type" required>
                                ...
                                <option value="pay_due_challan_bill_to_vehicle_provider">@lang('cmn.pay_due_challan_bill_to_vehicle_provider')</option>
                                @endif
                                @if($trip->provider->demarage_due > 0)
                                <option value="pay_due_demurrag_bill_to_vehicle_provider">@lang('cmn.payment_of_outstanding_demurrage_bill_to_vehicle_provider')</option>
                                @endif
                            </select>
