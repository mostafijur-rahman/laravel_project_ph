<tr>
    <td class="text-left">
        <small>
            ...
            <div class="row mt-2">
                <div class="btn-group">
                    ...
                    @if(Auth::user()->role->edit)
                        <a href="{{ url('trips/own-vehicle-single?page_name=edit&type='. $trip->type .'&trip_id='. $trip->id) }}" class="btn btn-primary btn-xs mr-1" aria-label="@lang('cmn.edit')">@lang('cmn.edit')</a>
                    @endif
                    <a href="{{ url('trips/own-vehicle-single?page_name=copy&type='. $trip->type .'&trip_id='. $trip->id) }}" class="btn btn-primary btn-xs mr-1" aria-label="@lang('cmn.copy')">@lang('cmn.copy')</a>
            ...
            <div class="row mt-2">
                <div class="btn-group">
                    <a href="{{ url('trips?page_name=demarage&type='. $trip->type .'&trip_id='. $trip->id) }}" class="btn btn-primary btn-xs mr-1" aria-label="@lang('cmn.demurrage')">@lang('cmn.demurrage')</a>
                    ...
                    <a href="{{ url('trips?page_name=oil_expense&type='. $trip->type .'&trip_id='. $trip->id) }}" class="btn btn-xs mr-1 btn-primary" aria-label="@lang('cmn.oil_expense')">@lang('cmn.oil_expense')</a>
                    <a href="{{ url('trips?page_name=meter&type='. $trip->type .'&trip_id='. $trip->id) }}" class="btn btn-xs mr-1 btn-primary" aria-label="@lang('cmn.meter_info')">@lang('cmn.meter_info')</a>
                </div>
            </div>
            <div class="row mt-2">
                <div class="btn-group">
                    @if(!$trip->challanHistoryReceived)
                    <button class="btn btn-primary btn-xs mr-1" onclick="challanReceived({{ json_encode($trip) }})" title="@lang('cmn.challan_received')">@lang('cmn.challan_received')</button>
                    @endif
    ...
    <td class="text-left">
        <small>
            ...
            @if($trip->meter->previous_reading)
                ...
                @lang('cmn.used_fuel'): <b>{{ number_format($tripOilLiterSumByGroupId) }}</b> @lang('cmn.liter')
                <br>

                @if($tripOilLiterSumByGroupId > 0)
                    @lang('cmn.mileage') @lang('cmn.km'): <b>{{ number_format(($trip->meter->current_reading - $trip->meter->previous_reading)/$tripOilLiterSumByGroupId, 2) }}</b>
                    <br>
                @endif
            @endif
    ...
    <td class="text-left">
        <small>
            ...
            @lang('cmn.weight'): <b>{{ number_format($trip->weight) }} {{ __('cmn.' . $trip->unit->name) }}</b>
            <br>
    ...
    <td class="text-right">
        ...
        @else
            <small>
                <div style="border-bottom: 2px dashed grey; margin-left: 16px;">
                    @lang('cmn.total_deposit') = <b>---</b><br>
                    @lang('cmn.total_demurrage') = <b>---</b><br>
                    @lang('cmn.total_expense') = <b>---</b><br>
                </div>
                @lang('cmn.balance') = <b>---</b><br>
            </small>
        @endif
    </td>
    <td class="text-right">
