                ...
                <table class="table table-bordered table-striped text-center table-hover text-nowrap">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <small>
                                    {{ $oil_expense->pump->name??__('cmn.cash_paid') }}
                                    @if($oil_expense->note){{ $oil_expense->note }}@endif
                                </small>
                            </td>
                            <td>
                                <small>{{ date('d M, Y', strtotime($oil_expense->date)) }}</small>
                            </td>
                            <td>
                            ...
                            <td>
                                <button type="button" class="btn btn-xs bg-gradient-danger" onclick="return deleteCertification({{ $oil_expense->id }}, 'oil')" title="@lang('cmn.delete')">@lang('cmn.delete')</button>
                        ...
                        <tr style="font-weight: bold;">
                            <td class="text-right" colspan="2">
                                <small><b>@lang('cmn.total') =</b></small>
                            </td>
                            <td>
                                <small><b>{{ number_format($total_oil_liter) }}</b></small>
                            </td>
                            <td></td>
                            <td>
                                <small><b>{{ number_format($total_oil_bill) }}</b></small>
                            </td>
                            <td></td>
                        </tr>
                    </tbody>
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <div class="input-group date" id="reservationdate" data-target-input="nearest">
                                    <input type="text" name="trip_date" value="{{ old('trip_date', date('d/m/Y')) }}" placeholder="@lang('cmn.date')" class="form-control datetimepicker-input {{ $errors->has('trip_date') ? 'is-invalid' : '' }}" data-target="#reservationdate" required>
                                    <div class="input-group-append" data-target="#reservationdate" data-toggle="datetimepicker">
                                        <div class="input-group-text"><i class="fa fa-calendar"></i></div>
                                    </div>
                                    @if ($errors->has('trip_date'))
                                        <span class="error invalid-feedback">{{ $errors->first('trip_date') }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <select class="form-control select2 {{ $errors->has('account_id') ? 'is-invalid' : '' }}" style="width: 100%;" name="account_id">
                                    ...
                                    @endforeach
                                    @endif
                                    <option value="loan">@lang('cmn.loan')</option>
                                </select>
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control {{ $errors->has('rate') ? 'is-invalid' : '' }}" name="rate" min="0" value="@setting('client_system.oil_rate')" step="any" class="form-control" placeholder="@lang('cmn.rate')" required>
                                @if ($errors->has('rate'))
