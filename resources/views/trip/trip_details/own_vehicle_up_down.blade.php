                    ...
                    <tr class="text-center">
                        <td class="text-left">
                            <small>
                                @if($up_trip->meter->previous_reading)
                                    ...
                                    @php $tripOilLiterSumByTripId = tripOilLiterSumByTripId($up_trip->id); @endphp 
                                ...
                                @if($up_trip->challanHistoryReceived)
                                    ...
                                    <button type="button" class="btn btn-xs btn-danger"  onclick="return deleteChallanReceivedCertification({{ $up_trip->challanHistoryReceived->id  }})" title="@lang('cmn.delete_challan_received')">@lang('cmn.delete_challan_received')</button>
                                    <form id="delete-challan-received-form-{{ $up_trip->challanHistoryReceived->id }}" method="POST" action="{{ url('trips/challan-received', $up_trip->challanHistoryReceived->id) }}" style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                ...
                                <div class="row mt-2">
                                    <div class="btn-group">
                                        <a href="{{ url('trips?page_name=print&type='. $up_trip->type .'&group_id='. $up_trip->group_id) }}" target="_blank" class="btn btn-default btn-xs mr-1" aria-label="@lang('cmn.print')">@lang('cmn.print')</a>
                                        ...
                                        <a href="{{ url('trips?page_name=details&type='. $up_trip->type .'&group_id='. $up_trip->group_id) }}" class="btn btn-xs mr-1 {{ ($request['page_name'] == 'details')?'btn-primary':'btn-default' }}" aria-label="@lang('cmn.details')">@lang('cmn.details')</a>
                                        <a href="{{ url('trips?page_name=transection&type='. $up_trip->type .'&group_id='. $up_trip->group_id) }}" class="btn btn-xs mr-1 {{ ($request['page_name'] == 'transection')?'btn-primary':'btn-default' }}" aria-label="@lang('cmn.transection')">@lang('cmn.transection')</a>
                                    </div>
                                </div>
                                <div class="row mt-2">
                                    <div class="btn-group">
                                        <a href="{{ url('trips?page_name=demarage&type='. $up_trip->type .'&group_id='. $up_trip->group_id) }}" class="btn btn-xs mr-1 {{ ($request['page_name'] == 'demarage')?'btn-primary':'btn-default' }}" aria-label="@lang('cmn.demurrage')">@lang('cmn.demurrage')</a>
                                        <a href="{{ url('trips?page_name=general_expense&type='. $up_trip->type .'&group_id='. $up_trip->group_id) }}" class="btn btn-xs mr-1 {{ ($request['page_name'] == 'general_expense')?'btn-primary':'btn-default' }}" aria-label="@lang('cmn.general_expense')">@lang('cmn.general_expense')</a>
                                        <a href="{{ url('trips?page_name=oil_expense&type='. $up_trip->type .'&group_id='. $up_trip->group_id) }}" class="btn btn-xs mr-1 {{ ($request['page_name'] == 'oil_expense')?'btn-primary':'btn-default' }}" aria-label="@lang('cmn.oil_expense')">@lang('cmn.oil_expense')</a>
                                        <a href="{{ url('trips?page_name=meter&type='. $up_trip->type .'&group_id='. $up_trip->group_id) }}" class="btn btn-xs {{ ($request['page_name'] == 'meter')?'btn-primary':'btn-default' }}" aria-label="@lang('cmn.meter_info')">@lang('cmn.meter_info')</a>
                                    </div>
                                </div>
                                <div class="row mt-2">
                                    <div class="btn-group">
                                        @if(!$up_trip->challanHistoryReceived)
                                            <button class="btn btn-default mr-1 btn-xs" onclick="challanReceived({{ json_encode($up_trip) }})" title="@lang('cmn.challan_received')">@lang('cmn.challan_received')</button>
                                        @endif
                                        @if(Auth::user()->role->delete)
                                            <button type="button" class="btn btn-xs btn-danger"  onclick="return deleteCertification({{ $up_trip->id  }})" title="@lang('cmn.delete')">@lang('cmn.delete')</button>
                                            <form id="delete-form-{{$up_trip->id }}" method="POST" action="{{ url('trip-delete-all', $up_trip->id ) }}" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        @endif
                        ...
                        <td class="text-left">
                            <small>
                                ...
                                <div class="row mt-2">
                                    ...
                                    <a href="{{ url('trips/own-vehicle-up-down?page_name=edit&type='. $up_trip->type .'&trip_id='. $trip->id) }}" class="btn btn-primary btn-xs" aria-label="@lang('cmn.edit')">@lang('cmn.edit')</a>
                                    @endif
                                </div>
                            </small>
                        </td>
                        ...
                        <td class="text-left">
                            ...
                            <small>
                                ...
                                <div class="row mt-2">
                                    @if(Auth::user()->role->edit)
                                    <a href="{{ url('trips/own-vehicle-up-down?page_name=edit&type='. $down_trip->type .'&trip_id='. $down_trip->id) }}" class="btn btn-primary btn-xs" aria-label="@lang('cmn.edit')">@lang('cmn.edit')</a>
                                    @endif
                                </div>
                            </small>
                            @else
                                ...
                                <br>
                                <a href="{{ url('trips/own-vehicle-up-down?page_name=create&type=own_vehicle_up_down&group_id='. $up_trip->group_id . '&number=' . $up_trip->number) }}" class="btn btn-xs btn-primary" aria-label="@lang('cmn.meter_info')"><i class="fa fa-plus"></i> @lang('cmn.add_challan')</a>
                            @endif
                        </td>
