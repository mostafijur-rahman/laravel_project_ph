...
@if($trip)
    @include('trip.demurrage_form.own_vehicle_up_down_list')

    <div class="col-md-12">
        <div class="card card-default">
            <form action="{{ url('trips/demurrage-save-for-own-vehicle') }}" method="POST" id="ex_form">
                @csrf
                <input type="hidden" name="trip_id" value="{{ $up_trip->id }}">
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="form-group">
                                <label>@lang('cmn.type')</label>
                                <select  class="form-control {{ $errors->has('challan_type') ? 'is-invalid' : '' }}" style="width: 100%;" name="challan_type" required>
                                    <option value="up">@lang('cmn.up') @lang('cmn.challan')</option>
                                    @if($down_trip && $down_trip->id)
                                    <option value="down">@lang('cmn.down') @lang('cmn.challan')</option>
                                    @endif
                                </select>
                                @if ($errors->has('challan_type'))
                                    <span class="error invalid-feedback">{{ $errors->first('challan_type') }}</span>
                                @endif
                            </div>
                        </div>
                        <div class="col-md-3">
