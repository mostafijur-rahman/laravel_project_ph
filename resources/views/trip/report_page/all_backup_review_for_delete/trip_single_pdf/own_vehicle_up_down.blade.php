    ...
    <table class="table-design" cellpadding="0" cellspacing="0">
        ...
        <tr class="text-center">
            <td class="text-left">
                ...
                <br>
                @lang('cmn.posting_date'): <b>{{ date('d M, Y', strtotime($up_trip->account_take_date)) }}</b><br>
                ...
                <b>{{ $up_trip->user->first_name}} ({{ date('d M, Y H:m A', strtotime($up_trip->created_at)) }})</b>
                @if($up_trip->updated_at > $up_trip->created_at)
                    <br>
                    @if($up_trip->updated_by)
                        @lang('cmn.post_updated_by'): <br>
                        <b>{{ $up_trip->user_update->first_name}} ({{ date('d M, Y H:m A', strtotime($up_trip->updated_at)) }})</b>
                    @endif
                @endif
                <br>
            ...
            <td class="text-left">
                ...
                @lang('cmn.vehicle'): <b>{{ $up_trip->provider->vehicle->vehicle_number }}</b>
                <br>
                ...
                @if($up_trip->box)
                @lang('cmn.box_qty'): <b>{{ number_format($up_trip->box) }}</b> টি
                <br>
                @endif

                @if($up_trip->weight)
                @lang('cmn.weight'): <b>{{ number_format($up_trip->weight) }}</b> {{ $up_trip->unit->name }}
                <br>
                @endif
                
                <hr>

                <b>{{ $up_trip->company->company->name }}</b><br>
                ...
                @if($up_trip->company->due_fair>0)
                    @lang('cmn.challan_due'): <b>{{ number_format($up_trip->company->due_fair) }}</b>
                    <br>
                @else
                    @lang('cmn.challan_due'): <b>{{ number_format($up_trip->company->due_fair) }}</b>
                    <br>
                ...
                {{-- @if($up_trip->company->demarage > $up_trip->company->demarage_received)
                    @lang('cmn.demarage_charge'): <b>{{ number_format($up_trip->company->demarage) }}</b>
                    <br>
                @else
                    @lang('cmn.demarage_charge'): <b>{{ number_format($up_trip->company->demarage) }}</b>
                    <br>
                @endif
                @lang('cmn.demarage_received'): <b>{{ number_format($up_trip->company->demarage_received) }}</b>
                <br>
                @lang('cmn.demarage_due'): <b>{{ number_format($up_trip->company->demarage_due) }}</b>
                <br> --}}
            ...
            <td class="text-left">
                ...
                @lang('cmn.vehicle'): <b>{{ $down_trip->provider->vehicle->vehicle_number }}</b>
                <br>
                ...
                @if($down_trip->box)
                @lang('cmn.box_qty'): <b>{{ number_format($down_trip->box) }}</b> টি
                <br>
                @endif

                @if($down_trip->weight)
                @lang('cmn.weight'): <b>{{ number_format($down_trip->weight) }}</b> {{ $down_trip->unit->name }}
                <br>
                @endif
                
                <hr>

                <b>{{ $down_trip->company->company->name }}</b>
                <br>
                @lang('cmn.contract_rent'): <b>{{ number_format($down_trip->company->contract_fair) }}</b>
                <br>
                @lang('cmn.addv_recev'): <b>{{ number_format($down_trip->company->advance_fair) }}</b>
                <br>
                
                @if($down_trip->company->due_fair>0)
                    @lang('cmn.challan_due'): <b>{{ number_format($down_trip->company->due_fair) }}</b>
                    <br>
                @else
                    @lang('cmn.challan_due'): <b>{{ number_format($down_trip->company->due_fair) }}</b>
                    <br>
                ...
                {{-- @if($down_trip->company->demarage > $down_trip->company->demarage_received)
                    @lang('cmn.demarage_charge'): <b>{{ number_format($down_trip->company->demarage) }}</b>
                    <br>
                @else
                    @lang('cmn.demarage_charge'): <b>{{ number_format($down_trip->company->demarage) }}</b>
                    <br>
                ...
                @lang('cmn.demarage_received'): <b>{{ number_format($down_trip->company->demarage_received) }}</b>
                <br>
                @lang('cmn.demarage_due'): <b>{{ number_format($down_trip->company->demarage_due) }}</b>
                <br> --}}
            ...
            @if($up_trip)
                @php
                    ...
                    $tripOilLiterSumByGroupId = tripOilLiterSumByTripId($up_trip->id); // string

                    $trip_general_exp_lists = tripExpenseListSumByTripId($up_trip->id); // array

                    $total_general_expense_sum = tripExpenseSumByTripId($up_trip->id); // string
                    $total_oil_bill_sum = tripOilBillSumByTripId($up_trip->id); // string
                    $total_expense = number_format($total_general_expense_sum + $total_oil_bill_sum);

                @endphp
                <td class="text-right">
                    @lang('cmn.total_deposit') = <b>{{ number_format($total_received_rent) }}</b>
                    <br>
                    @if($up_trip->company->demarage_received > 0 || $down_trip->company->demarage_received > 0)
                        @lang('cmn.total_demarage') = (+) <b>{{ number_format($up_trip->company->demarage_received + $down_trip->company->demarage_received) }}</b>
                        <br>
                    @endif
                    @lang('cmn.total_expense') = (-) <b>{{ $total_expense }}</b>
                    <br>
                    <hr>
                    @lang('cmn.net_income') = <b>{{ $net_income = number_format(($total_received_rent + ($up_trip->company->demarage_received + $down_trip->company->demarage_received)) - ($total_general_expense_sum + $total_oil_bill_sum)) }}</b>
                </td>
                <td class="text-right">
                    ...
                    @if($trip_general_exp_lists)
                        ...
                        @foreach($trip_general_exp_lists as $trip_general_exp_list)
                        <b>{{ $trip_general_exp_list->head }} = </b> {{ number_format($trip_general_exp_list->trip_single_expense_sum) }}
                        <br>
                        @endforeach
                        <hr>
                        <b>@lang('cmn.total_expense') = {{ $total_expense }}</b>
                    @endif
                </td>
            @endif
        </tr>
