				...
				<table class="table-design" cellpadding="0" cellspacing="0">
                    ...
                    @if(count($lists) > 0)
                        ...
                        @foreach($lists as $key => $list)
                            @php 
                                $contract_fair_sum += $list->company->contract_fair; 
                                $advance_fair_sum += $list->company->advance_fair; 
                                $due_fair_sum += $list->company->due_fair;

								if($request->demarage_status){
									$demarage_bill_sum += $list->company->demarage; 
									$demarage_received_sum += $list->company->demarage_received; 
									$demarage_due_sum += $list->company->demarage_due;
								}	
							@endphp
							<tr class="item">
