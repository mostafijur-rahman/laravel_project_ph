					...
					<thead>
						<tr>
							...
							@if(count($ranges) > 0)
								...
								@switch($request->date_range_status)
				
									@case('monthly_report')
									@case('date_wise')
										<td class="center">@lang('cmn.date')</td>
										@break
				
									@case('yearly_report')
										<td class="center">@lang('cmn.month')</td>
										@break
						...
						@foreach($ranges as $range_key => $range)
							...
							<tr class="item">
								...
								@switch($request->date_range_status)
									...
									@case('monthly_report')
									@case('date_wise')
										<td class="center">{{ $range->format('d/m/Y') }}</td>
										@break
			
									@case('yearly_report')
										<td class="center">@lang('cmn.'. $range)</td>
										@break
								...
								@if(count($setting_expenses) > 0)
									@foreach($setting_expenses as $expense_key => $setting_expense)
										...
										@switch($request->date_range_status)
											...
											@case('date_wise')
												@php
												$amount = getExpenseSumByAccordingToRange($setting_expense->id, $range, $request);
												@endphp
												@break
			
											@case('yearly_report')
												@php
												$amount = getExpenseSumByAccordingToRange($setting_expense->id, $range_key, $request);
												@endphp
												@break
