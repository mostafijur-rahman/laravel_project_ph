                    ...
                    <tbody>
                        ...
                        <tr align="center">
                            ...
                            <td>
                                <small>
                                    @if($list->trip_id)
                                        @lang('cmn.challan_no') : <strong>{{ $list->trip->number }}</strong>
                                    @else
                                        @lang('cmn.challan_no') : ---
                                    @endif
                                    <br>
                                    ...
                                    <br>
                                    @if($list->pump_id)
                                        @lang('cmn.pump') : <strong>{{ $list->pump->name }}</strong>
                                    @else
                                        @lang('cmn.pump') : <strong>@lang('cmn.cash_paid')</strong>
                                    @endif
                                    <br>
                                    @if($list->voucher_id)
                            ...
                            <td>
                                <strong>{{ number_format($list->liter) }}</strong>
                                @php $total_liter += $list->liter @endphp
                            </td>
                            <td>
                                <strong>{{ number_format($list->rate) }}</strong>
                                @php $total_rate += $list->rate @endphp
                            </td>
                            <td>
                                <strong>{{ number_format($list->bill) }}</strong>
                                @php $total_bill += $list->bill @endphp
                            </td>
                            <td>
                                @if($list->trip_id)
                                ---
                                @else
                                    ...
                                    @if(Auth::user()->role->delete)
                                        ...
                                        <form id="delete-form-{{$list->id }}" method="POST" action="{{ url('oil-expenses', $list->id ) }}" style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    @endif
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="3" class="text-right"><b>@lang('cmn.total')=</b></td>
                                <td><b>{{ number_format($total_liter) }}</b></td>
                                <td><b>{{ number_format($total_rate) }}</b></td>
                                <td><b>{{ number_format($total_bill) }}</b></td>
                                <td></td>
                            </tr>
                        </tfoot>
                        @else
                        <tr>
                            <td colspan="7" class="text-center text-red"><h4>@lang('cmn.empty_table')</h4>
                        </tr>
                        @endif
                    </tbody>
                </table>
...
// from oil list
    ...
    function editData(value){
        ...
        $("#pump_id").val(value.pump_id);
        $("#voucher_id").val(value.voucher_id);
        $("#liter").val(value.liter);
        $("#rate").val(value.rate);
        $("#bill").val(value.bill);
        $("#note").val(value.note);
        
        $('#aciton_edit').attr('action', '');
        $('#aciton_edit').attr('action', base_url + '/oil-expenses/' + value.id);
